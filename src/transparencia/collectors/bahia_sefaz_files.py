...
def download_ckan_resource(
    ...
    *,
    ...
    """Baixa um recurso somente do host oficial do CKAN da Bahia.

    Se o runner não validar a cadeia TLS do mesmo host oficial, repete a requisição
    com verify=False e registra explicitamente essa condição na evidência.
    """
    parts = urlsplit(url)
...
def summarize_sefaz_revenues(path: Path, *, target_year: int | None = None) -> dict[str, Any]:
    ...
    agency_field = _first_header(headers, (("ORGAO",), ("UNIDADE", "ORCAMENT")), reject=("COD", "SIGLA"))
    ...
    natures: dict[str, dict[str, float]] = defaultdict(lambda: {"rows": 0.0, "realized": 0.0})
        ...
        for row in reader:
            ...
            if target_year is None or row_year == target_year:
                ...
                if agency_field:
                    ...
                    agencies[agency]["realized"] += realized
                if nature_field:
                    nature = str(row.get(nature_field) or "Não informado").strip() or "Não informado"
                    natures[nature]["rows"] += 1
                    natures[nature]["realized"] += realized
    finally:
        handle.close()
    ...
    def top(items: dict[str, dict[str, float]]) -> list[dict[str, Any]]:
        ...
        return [
            {"name": name, "rows": int(values["rows"]), "realized": round(values["realized"], 2)}
    ...
    return {
        ...
        "totals_by_year": {
            str(key): {name: round(value, 2) for name, value in values.items()}
            for key, values in sorted(totals_by_year.items(), key=lambda item: str(item[0]))
        },
        ...
        "top_revenue_natures_by_realized": top(natures) if value_fields["realized"] else [],
...
def _classify_licitacao_table(member: str, headers: Iterable[str]) -> str:
    ...
    name = _norm(member)
    if "PARTICIP" in joined or "FORNECEDOR" in joined or "LICITANTE" in joined:
...
def _summarize_zip_member(zf: zipfile.ZipFile, member: str, *, target_year: int | None) -> dict[str, Any]:
    ...
    agency_field = _first_header(headers, (("ORGAO",), ("UNIDADE",)), reject=("COD", "CPF", "CNPJ"))
    year_field = _first_header(headers, (("ANO",), ("EXERCICIO",)), reject=("COD",))
    date_field = _first_header(headers, (("DATA", "HOMOLOG"), ("DATA", "ABERT"), ("DATA",)), reject=("ATUALIZ",))
    value_fields = [header for header in headers if "VALOR" in _norm(header) and not any(token in _norm(header) for token in ("COD", "PERCENT"))][:12]
    ...
    with zf.open(member, "r") as raw:
        ...
        for row in reader:
            ...
            in_scope = target_year is None or row_year is None or row_year == target_year
            if not in_scope:
                continue
            selected_rows += 1
    ...
    privacy_fields = [header for header in headers if any(token in _norm(header) for token in ("CPF", "CNPJ", "PARTICIPANTE", "LICITANTE", "FORNECEDOR"))]
    return {
        "member": member,
        ...
        "value_field_sums": {
            field: {"sum": round(value_totals[field], 2), "numeric_rows": numeric_counts[field]}
            for field in value_fields
        },
    }
...
def summarize_sefaz_licitacoes_zip(path: Path, *, target_year: int | None = None) -> dict[str, Any]:
    ...
    with zipfile.ZipFile(path, "r") as zf:
        ...
        csv_members = [
            ...
            if info.filename.lower().endswith((".csv", ".txt")) and info.file_size > 0
        ]
        if not csv_members:
    ...
    return {
        ...
        "selected_rows_across_related_tables": sum(table["selected_rows"] for table in tables),
        "interpretation": "As linhas pertencem a tabelas relacionadas e não são somadas como número de licitações. Valores são apresentados por campo/tabela; o projeto não combina automaticamente itens, participantes e certames.",
