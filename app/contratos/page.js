    ...
    <>
      ...
      <section className="section">
        <div className="shell">
          ...
          <div className="card table-card"><div className="table-wrap"><table><thead><tr><th>Contrato</th><th>Processo</th><th>Fornecedor</th><th>Unidade</th><th>Objeto</th><th>Valor atualizado/global</th><th>Vigência</th></tr></thead><tbody>{topContracts.map((row, index) => <tr key={`${row.id}-${index}`}><td><a href={row.fonte} target="_blank" rel="noreferrer"><strong>{row.numero || row.numeroSigef || "—"}</strong> ↗</a>{row.situacao && <div className="muted">{row.situacao}</div>}</td><td className="mono">{row.processo || "—"}</td><td>{row.documentoFornecedor ? <Link href={`/fornecedores/${encodeURIComponent(row.documentoFornecedor)}`}><strong>{row.fornecedor || "—"}</strong><div className="muted mono">{row.documentoFornecedor}</div></Link> : (row.fornecedor || (row.credorOmitidoPorPrivacidade ? "Omitido na grade municipal" : "—"))}</td><td>{row.unidade || row.orgao || "—"}</td><td className="object-cell">{row.objeto || "—"}</td><td><strong className="mono">{row.valorGlobal != null ? brl(row.valorGlobal) : "—"}</strong></td><td>{dateBR(row.vigenciaInicio)} → {dateBR(row.vigenciaFim)}</td></tr>)}</tbody></table></div></div>
