...
export default function BahiaContratosPage() {
  ...
  const procurementLinks = data.sefaz?.procurementContractLinks;
  const linkSummary = procurementLinks?.summary;
  const exactLinks = linkSummary?.exact_links ?? [];
  ...
  return <>
    ...
    {exactLinks.length > 0 && <section className="section"><div className="shell"><div className="section-head enxuto"><div><span className="eyebrow">Licitação → contrato</span><h2>Vínculos comprovados por identificador oficial</h2></div><p>O vínculo só é publicado quando o processo da tabela principal de licitações e o instrumento contratual aparecem relacionados nas próprias tabelas oficiais da SEFAZ/SIMPAS.</p></div><div className="grid grid-3"><div className="card stat accent"><span className="stat-label">Vínculos exatos</span><div><span className="stat-value">{integer(linkSummary.exact_link_count)}</span><div className="stat-meta">pares processo ↔ instrumento</div></div></div><div className="card stat"><span className="stat-label">Processos com instrumento</span><div><span className="stat-value">{integer(linkSummary.processes_with_instruments)}</span><div className="stat-meta">no recorte anual validado</div></div></div><div className="card stat blue"><span className="stat-label">Instrumentos únicos</span><div><span className="stat-value">{integer(linkSummary.unique_instruments)}</span><div className="stat-meta">identificadores normalizados</div></div></div></div><div className="compact-list">{exactLinks.slice(0, 50).map((item) => <div className="compact-row" key={`${item.process_id}-${item.instrument_id}`}><div><strong>Processo {item.process_id}</strong><span>identificador oficial normalizado</span></div><div><strong>Contrato {item.instrument_id}</strong><span>instrumento relacionado</span></div></div>)}</div>{exactLinks.length > 50 && <p className="muted">Mostrando 50 de {integer(exactLinks.length)} vínculos exatos no resumo público. A contagem total permanece disponível no conjunto auditável.</p>}<div className="notice"><span>i</span><div><strong>Sem inferência:</strong> sem identificador oficial correspondente, o projeto não cria vínculo por nome de fornecedor, objeto semelhante ou aproximação textual.</div></div></div></section>}
