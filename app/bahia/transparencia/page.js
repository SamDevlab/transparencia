...
export default function BahiaTransparenciaPage() {
  ...
  const sefazExpected = data.sefaz?.summary?.expected ?? 2;
  ...
  return <>
    ...
    {data.available && <section className="section compacto"><div className="shell grid grid-3"><div className="card stat accent"><span className="stat-label">Catálogos SEFAZ/CKAN</span><div><span className="stat-value">{integer(summary.ckan_datasets_collected)}/{integer(summary.ckan_datasets_expected)}</span><div className="stat-meta">fontes consultadas nesta coleta</div></div></div><div className="card stat"><span className="stat-label">Bases SEFAZ processadas</span><div><span className="stat-value">{integer(sefazProcessed)}/{integer(sefazExpected)}</span><div className="stat-meta">Receitas + Licitações prioritárias</div></div></div><div className="card stat blue"><span className="stat-label">Conjuntos TCE processados</span><div><span className="stat-value">{integer(summary.tce_datasets_processed)}</span><div className="stat-meta">enriquecimento separado; indisponibilidade não vira zero</div></div></div></div></section>}

    {revenueSummary && <section className="section"><div className="shell"><div className="section-head enxuto"><div><span className="eyebrow">SEFAZ / FIPLAN</span><h2>Receitas estaduais {revenueSummary.selected_year || ""}</h2></div><p>Previsão, valor atualizado e arrecadação permanecem separados. Os números são somas dos campos identificados no arquivo oficial.</p></div><div className="grid grid-4"><div className="card stat"><span className="stat-label">Linhas no arquivo</span><div><span className="stat-value">{integer(revenueSummary.rows)}</span><div className="stat-meta">arquivo oficial processado</div></div></div>{revenueSummary.schema?.detected_fields?.forecast && <div className="card stat"><span className="stat-label">Previsão</span><div><span className="stat-value">{brl(revenueTotals?.forecast, { compact: true })}</span><div className="stat-meta">{revenueSummary.schema.detected_fields.forecast}</div></div></div>}{revenueSummary.schema?.detected_fields?.updated && <div className="card stat"><span className="stat-label">Atualizada</span><div><span className="stat-value">{brl(revenueTotals?.updated, { compact: true })}</span><div className="stat-meta">{revenueSummary.schema.detected_fields.updated}</div></div></div>}{revenueSummary.schema?.detected_fields?.realized && <div className="card stat accent"><span className="stat-label">Arrecadada</span><div><span className="stat-value">{brl(revenueTotals?.realized, { compact: true })}</span><div className="stat-meta">{revenueSummary.schema.detected_fields.realized}</div></div></div>}</div><div className="results-line"><span>SHA-256 do arquivo original: <code>{shortHash(revenues.evidence?.sha256)}</code></span><span>Atualização da fonte: {revenues.resource?.last_modified || "não informada"}</span></div>{revenueSummary.top_agencies_by_realized?.length > 0 && <div className="section-subblock"><h3>Maiores valores arrecadados por órgão no recorte</h3><div className="compact-list">{revenueSummary.top_agencies_by_realized.slice(0, 10).map((item) => <div className="compact-row" key={item.name}><div><strong>{item.name}</strong><span>{integer(item.rows)} registro(s)</span></div><strong>{brl(item.realized)}</strong></div>)}</div></div>}</div></section>}

    {procurementSummary && <section className="section"><div className="shell"><div className="section-head enxuto"><div><span className="eyebrow">SEFAZ / SIMPAS</span><h2>Licitações estaduais processadas</h2></div><p>O arquivo contém tabelas relacionadas. O sistema não soma itens, participantes e certames como se fossem uma única “quantidade de licitações”.</p></div><div className="grid grid-3"><div className="card stat"><span className="stat-label">Tabelas tabulares</span><div><span className="stat-value">{integer(procurementSummary.archive?.tabular_members)}</span><div className="stat-meta">CSV/TXT dentro do ZIP</div></div></div><div className="card stat"><span className="stat-label">Linhas relacionadas</span><div><span className="stat-value">{integer(procurementSummary.total_rows_across_related_tables)}</span><div className="stat-meta">não equivale ao número de licitações</div></div></div><div className="card stat blue"><span className="stat-label">Linhas no recorte {procurementSummary.selected_year || ""}</span><div><span className="stat-value">{integer(procurementSummary.selected_rows_across_related_tables)}</span><div className="stat-meta">quando ano/data pôde ser identificado</div></div></div></div><div className="coverage-grid source-coverage-grid">{(procurementSummary.tables ?? []).slice(0, 12).map((table) => <article className="coverage-card source-card" key={table.member}><header><strong>{table.member}</strong><span className="badge green">{String(table.classification || "tabela relacionada").replaceAll("_", " ")}</span></header><p>{integer(table.rows)} linhas na tabela · {integer(table.selected_rows)} no recorte selecionado.</p>{table.top_modalities?.length > 0 && <p className="muted">Modalidades mais frequentes: {table.top_modalities.slice(0, 3).map((item) => `${item.name} (${integer(item.rows)})`).join(" · ")}</p>}<div className="source-foot"><span>{table.schema?.headers?.length ?? 0} colunas</span><span>{table.schema?.privacy_sensitive_columns_present?.length ? "contém colunas sensíveis; linhas não republicadas" : "sem coluna sensível detectada"}</span></div></article>)}</div><div className="results-line"><span>SHA-256 do ZIP original: <code>{shortHash(procurements.evidence?.sha256)}</code></span><span>Atualização da fonte: {procurements.resource?.last_modified || "não informada"}</span></div></div></section>}
