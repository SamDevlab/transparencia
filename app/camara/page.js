...
export default function CamaraPage() {
  ...
  const certamesComplete = auxiliary?.certames?.complete === true;
      ...
      {auxiliary && <section className="section">
        <div className="shell">
          ...
          <div className="grid grid-3">
            ...
            <div className="card panel"><div className="panel-title"><h3>Certames</h3><span>{certamesComplete ? "catálogo completo" : "parcial"}</span></div><div className="stat-value mono" style={{ fontSize: 28 }}>{integer(auxiliary.certames?.records ?? 0)}</div><p>{certamesComplete ? `${integer(auxiliary.certames?.pages ?? 0)} páginas percorridas; total reconciliado com os ${integer(auxiliary.certames?.serverReportedTotal ?? 0)} registros informados pelo servidor.` : "A cobertura ainda não comprovou simultaneamente paginação integral e total da fonte."}</p><p className="muted">{auxiliary.certames?.coverageRule}</p><a className="button" href="https://cmsalvador.sys.inf.br/ca/licitacao/" target="_blank" rel="noreferrer">Abrir certames →</a></div>
          </div>
