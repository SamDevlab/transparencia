    ...
    <div>
      ...
      {agency && <>
        <div className="fluxo-resumo">
          ...
          <div className="fluxo-conector">→</div>
          <div className="fluxo-no"><span>Vínculos exatos com PNCP</span><strong>{links.length}</strong><small>mostrados neste recorte</small></div>
        </div>

        {links.length > 0 ? <div className="relacoes-lista">{links.map((link) => link.contratos.map((contract, index) => <article className="relacao-dinheiro" key={`${link.processoId}-${contract.numero}-${index}`}><div className="relacao-coluna"><span>Processo</span><Link href={`/processos/${encodeURIComponent(link.processoId)}`}><strong className="mono">{link.processo || "—"}</strong></Link><small>{link.objeto}</small></div><div className="relacao-seta grande">→</div><div className="relacao-coluna"><span>Contrato</span><strong>{contract.numero || "PNCP"}</strong><small>{brl(contract.valorGlobal)}</small></div><div className="relacao-seta grande">→</div><div className="relacao-coluna"><span>Fornecedor</span><Link href={contract.documentoFornecedor ? `/fornecedores/${encodeURIComponent(contract.documentoFornecedor)}` : "/fornecedores"}><strong>{contract.fornecedor || "—"}</strong></Link><small className="mono">{contract.documentoFornecedor || ""}</small></div></article>))}</div> : <div className="notice warn"><span>!</span><div><strong>Nenhum vínculo exato exibido para este órgão.</strong> Isso não significa ausência de contratos. Significa apenas que, no recorte complementar PNCP preservado, não houve coincidência exata de número de processo com as aquisições municipais deste órgão.</div></div>}
      </>}
    </div>
  );
}
